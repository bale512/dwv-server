...
@cached_property
def myentropy(self):
    # sum of -plog(p) for all actions
    with chainer.force_backprop_mode():
        # self.all_prob: batch x action x a x b x h
        # self.all_log_prob: batch x action x a x b x h
        # return: batch x 1 x a x b x h
        return F.stack([- F.sum(self.all_prob * self.all_log_prob, axis=1)], axis=1)
#######################

###########################
def mylog_prob(self, x):
    # log(p) for selected action
    # x(action): batch x a x b x h
    n_batch, n_actions, a, b, h = self.all_log_prob.shape
    p_trans = F.transpose(self.all_log_prob, axes=(0,2,3,4,1))
    p_trans = F.reshape(p_trans,(-1,n_actions)) # (batch x a x b x h) x action 
    x_reshape = F.reshape(x,(1,-1))[0] # (batch x a x b x h)
    selected_p = F.select_item(p_trans,x_reshape) # (batch x a x b x h)
    return F.reshape(selected_p, (n_batch,1,a,b,h)) # batch x 1 x a x b x h
##########################
...
#######################
    ...
    def update(self, statevar, step=1):
...
##############################
            pi_loss -= log_prob * F.cast(advantage.data, 'float')
...
#########################
    ...
    def stop_episode_and_train(self, state, reward, step, done=False):
#########################
        #self.past_rewards[self.t - 1] = reward
        self.past_rewards[self.t - 1] = chainer.cuda.to_gpu(reward)
        if done:
            self.update(None, step=step)
        else:  
            #statevar = self.batch_states([state], np, self.phi)
            statevar = chainer.cuda.to_gpu(state)
########################
